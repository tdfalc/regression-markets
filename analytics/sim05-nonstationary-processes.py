...
def plot_coefficients(
    ...
    for i, ((experiment_title, experiment_config), ax) in enumerate(
        zip(
            experiments.items(),
            axs.flatten() if len(experiments) > 1 else [axs],
        )
    ):
        ...
        ax.set_xlim((-4, 104))
        prettify(ax=ax, legend=i == 0, legend_loc="upper left")
...
def main() -> None:
    ...
    savedir = Path(__file__).parent / "docs/sim05-nonstationary-processes"
