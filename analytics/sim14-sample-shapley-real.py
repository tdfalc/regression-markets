...
if __name__ == "__main__":
    ...
    savedir = Path(__file__).parent / "docs/sim14-sample-shapley"
    ...
    for SAMPLE in range(N):
        ...
        for i in range(len(num_replications)):
            ...
            old_output["train"][SAMPLE, i] = orig_output["train"][metric][i]
            old_output["test"][SAMPLE, i] = orig_output["test"][metric][i]

            new_output["train"][SAMPLE, i] = np.sum(
                output["train"][metric][i + k : i + k + num_replicates + 1]
            )

            new_output["test"][SAMPLE, i] = np.sum(
                output["test"][metric][i + k : i + k + num_replicates + 1]
            )
